 ...
 */
const normalizeInstitutionTypesResponse = (
  response: any,
): InstitutionType[] => {
  ...
  if (
    ...
    Array.isArray(response.institution_types)
  ) {
    console.log("✅ Response has institution_types array");
    return response.institution_types;
  }

  logger.warn("Unexpected institution types response format", {
...
export const useInstitutionTypes = ({
  ...
  return useQuery<{ success: boolean; institution_types: InstitutionType[] }>({
    ...
    queryFn: async () => {
      ...
      if (process.env.NODE_ENV === "development") {
        console.log("🔧 useInstitutionTypes Debug:", {
          ...
          fallbackTypes: fallbackTypes.map((t) => ({
            key: t.key,
            level: t.default_level,
            label: t.label_az,
          })),
          hasMinistryInFallback: fallbackTypes.some(
            (t) => t.key === "ministry",
          ),
        });
      }
      ...
      if (effectiveRole === "superadmin") {
        try {
          ...
          const normalizedTypes =
            normalizeInstitutionTypesResponse(apiResponse);
          ...
          if (process.env.NODE_ENV === "development") {
            console.log("📡 API Response for institution types:", {
              ...
              hasMinistryInAPI: normalizedTypes.some(
                (t) => t.key === "ministry",
              ),
            });
          }
          ...
          if (normalizedTypes.length > 0) {
            ...
            logger.warn("API returned empty types, using fallback", {
              component: "useInstitutionTypes",
            });
          ...
          logger.error("Failed to load institution types from API", {
            error: error,
            component: "useInstitutionTypes",
            action: "api-fallback",
          });
    ...
    enabled: enabled,
    staleTime: 1000 * 60 * 1, // Cache for 1 minute (temporarily reduced for testing)
