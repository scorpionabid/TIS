...
export const RegionClassManagement = () => {
  ...
  const [activeTab, setActiveTab] = useState<"classes" | "academic-years">(
    "classes",
  );
  ...
  const filterParams: ClassFilters = useMemo(() => {
    ...
    // Only add region_id for SuperAdmin - RegionAdmin should only see their own region
  ...
  useEffect(() => {
    if (waitingForRegionSelection) {
      console.log(
        "[RegionClassManagement] Waiting for region selection (superadmin)",
      );
      return;
    }
  ...
  useEffect(() => {
    if (classesErrorMessage) {
      console.error(
        "[RegionClassManagement] Classes query error",
        classesError,
      );
      toast({
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      const message =
        error?.response?.data?.message || error?.message || "Sinif yenilənmədi";
      toast({
        title: "Xəta",
        description: message,
        variant: "destructive",
  ...
  const handleConfirmDelete = async () => {
    ...
    const classLabelBase =
      `${deleteTarget.class_level ?? ""}${deleteTarget.name ?? ""}`.trim();
  ...
  return (
    <div className="p-6">
      <Tabs
        value={activeTab}
        onValueChange={(value) =>
          setActiveTab(value as "classes" | "academic-years")
        }
        className="space-y-6"
      >
        ...
        <TabsContent value="classes" className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {isSuperAdmin && waitingForRegionSelection
                    ? "..."
                          ...
                          >
                            ...
                            <td className="p-3">
                              {cls.education_program ? (
                                <Badge variant="outline" className="text-xs">
                                  {cls.education_program === "umumi"
                                    ? "Ümumi"
                                    : cls.education_program === "xususi"
                                      ? "Xüsusi"
                                      : cls.education_program === "ferdi_mekteb"
                                        ? "Fərdi (M)"
                                        : cls.education_program === "ferdi_ev"
                                          ? "Fərdi (E)"
                                          : cls.education_program}
                                </Badge>
                              ) : (
                            ...
                            <td className="p-3 text-center">
                              <Badge
                                variant={
                                  cls.is_active ? "default" : "secondary"
                                }
                                className={
          ...
          >
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {selectedClasses.length} sinifi silmək üzrəsiniz. Bu əməliyyat
