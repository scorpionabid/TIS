...
export default function RegionAttendanceReports() {
  ...
  useEffect(() => {
    ...
    if (selectedSchoolId) {
      const exists = schools.some(
        (school) => school.school_id === Number(selectedSchoolId)
      );
      if (!exists) {
  ...
  const filteredSchoolOptions = useMemo(() => {
    ...
    return schools.filter(
      (school) => school.sector_id === Number(selectedSectorId)
    );
  ...
  return (
    <div className="px-2 py-4 sm:px-4 space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-bold">
            Regional Davamiyyət Hesabatları
          </h1>
      ...
      <Tabs
        value={activeTab}
        onValueChange={(value) => setActiveTab(value as "overview" | "classes")}
      >
              ...
              <CardContent className="space-y-4">
                {overviewFetching && !overview ? (
                  ...
                  <p className="text-sm text-muted-foreground">
                    Seçilmiş filterlər üçün sektor tapılmadı.
                  </p>
                ) : (
                  <Table>
                    <TableHeader>
              ...
              <CardContent className="space-y-3">
                <div>
                  <h4 className="font-semibold text-sm">
                    Məlumat göndərməyən məktəblər
                  </h4>
                  <div className="mt-2 space-y-1">
                    {(overview?.alerts.missing_reports ?? [])
                      .slice(0, 5)
                      .map((alert) => (
                        <div
                          key={alert.school_id}
                    ...
                    {!overview?.alerts.missing_reports?.length && (
                      <p className="text-xs text-muted-foreground">
                        Bütün məktəblər hesabat göndərib.
                      </p>
                    )}
                  </div>
                </div>
                <div>
                  ...
                  <div className="mt-2 space-y-1">
                    {(overview?.alerts.low_attendance ?? [])
                      .slice(0, 5)
                      .map((alert) => (
                        <div
                          key={alert.school_id}
                    ...
                    {!overview?.alerts.low_attendance?.length && (
                      <p className="text-xs text-muted-foreground">
                        Aşağı davamiyyət müşahidə edilmir.
                      </p>
                    )}
                  </div>
                </div>
                ...
                {classLoading || classFetching ? (
                  ...
                  <>
                    <Card>
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold">
                          {classBreakdown.summary.total_classes}
                        </div>
                        <p className="text-xs text-muted-foreground">
                          {classBreakdown.summary.reported_classes} sinif
                          hesabat göndərib
                        </p>
                      </CardContent>
                    </Card>
                    <Card>
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold">
                          {formatPercent(
                            classBreakdown.summary.average_attendance_rate
                          )}
                        </div>
                        <p className="text-xs text-muted-foreground">
                          Seçilmiş dövr üzrə
                    ...
                    <Card>
                      ...
                      <CardContent>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {classBreakdown.summary.period.school_days} tədris
                          günü
                        </p>
                      </CardContent>
                    </Card>
                      ...
                      <TableBody>
                        {classBreakdown.classes.map((classStat) => (
                          <TableRow key={classStat.grade_id ?? classStat.name}>
                            ...
                            <TableCell className="text-center">
                              {classStat.reported_days}/
                              {classStat.expected_school_days}
                            </TableCell>
                            <TableCell className="text-center font-semibold">
