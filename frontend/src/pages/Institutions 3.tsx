...
const Institutions = () => {
  ...
  const fetchInstitutionAdmin = async (institutionId: number) => {
    ...
    try {
      ...
      const adminRoles = ['məktəbadmin', 'schooladmin', 'regionadmin', 'sektoradmin'];
      let admin = null;
                  ...
                  <TableRow key={institution.id} className="hover:bg-muted/50">
                    ...
                    <TableCell>
                      <div className="max-w-[150px] truncate">
                        {institutionAdmins[institution.id] ? (
                          <span title={`${institutionAdmins[institution.id]?.first_name} ${institutionAdmins[institution.id]?.last_name}`}>
                            {institutionAdmins[institution.id]?.first_name} {institutionAdmins[institution.id]?.last_name}
                          </span>
                        ) : (
