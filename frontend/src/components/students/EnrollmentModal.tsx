...
export const EnrollmentModal: React.FC<EnrollmentModalProps> = ({
  ...
  const getCapacityBadge = (pct: number) => {
    if (pct >= 100) return { label: 'Dolu', cls: 'bg-red-100 text-red-700 border-red-200' };
    if (pct >= 90) return { label: 'Həddə yaxın', cls: 'bg-orange-100 text-orange-700 border-orange-200' };
    return { label: 'Boş var', cls: 'bg-green-100 text-green-700 border-green-200' };
  };
                ...
                <div className="max-h-52 overflow-y-auto space-y-2 pr-1">
                  {sortedClasses.length > 0 ? (
                    sortedClasses.map((cls) => {
                      ...
                      const badge = getCapacityBadge(pct);
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            <div className="text-right">
                              ...
                              <div className="w-20 bg-gray-200 rounded-full h-1.5 mt-1">
                                <div
                                  className={`h-1.5 rounded-full ${pct >= 100 ? 'bg-red-500' : pct >= 90 ? 'bg-orange-400' : 'bg-green-500'}`}
