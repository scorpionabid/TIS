...
export const ModernRegionOperatorDashboard = memo(() => {
  ...
  const [newEvent, setNewEvent] = React.useState({ title: '', type: 'task', time: '09:00', link: '' });
  
  const [myEvents, setMyEvents] = React.useState(() => {
    const saved = localStorage.getItem('tis_operator_events');
    if (saved) {
      try {
    ...
    return [
      { id: 1, date: new Date(), title: "Gündəlik sənəd yoxlanışı", type: "task", time: "09:00", link: "" },
      { id: 2, date: new Date(), title: "Həftəlik hesabatın hazırlanması", type: "report", time: "16:00", link: "" }
    ];
  });

  // Notes State
  const [notes, setNotes] = React.useState(() => {
    const saved = localStorage.getItem('tis_operator_notes');
  ...
  React.useEffect(() => {
    localStorage.setItem('tis_operator_events', JSON.stringify(myEvents));
  }, [myEvents]);

  React.useEffect(() => {
    localStorage.setItem('tis_operator_notes', JSON.stringify(notes));
  }, [notes]);
  ...
  const handleAddEvent = () => {
    ...
    setNewEvent({ title: '', type: 'task', time: '09:00', link: '' });
  ...
  return (
    <ModernDashboardWrapper showGreeting={true}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 mb-8">
        <ModernStatsCard
          title="Tapşırıqlar"
          value={dashboard?.overview?.tasks?.total || 0}
          icon={ClipboardList}
        ...
        <ModernStatsCard
          title="Sorğular"
          value={dashboard?.overview?.surveys?.awaiting_approval || 0}
          icon={CheckCircle2}
          trend={{ value: 0, isPositive: true }}
          color="emerald"
        />
        <ModernStatsCard
          title="Sənədlər"
          value={dashboard?.overview?.documents?.uploaded_this_month || 0}
          icon={FileText}
          trend={{ value: 0, isPositive: true }}
          color="violet"
        />
        <ModernStatsCard
          title="Linklər"
          value={dashboard?.overview?.links?.total_active || 0}
          icon={LinkIcon}
          trend={{ value: 0, isPositive: true }}
          color="amber"
        />
      ...
      <Dialog open={isEventModalOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="sm:max-w-[425px] rounded-3xl">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              <Label htmlFor="link">Link (istəyə bağlı)</Label>
              <Input
                id="link"
                placeholder="https://..."
                value={newEvent.link}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="grid gap-2">
                ...
                <Input id="time" type="time" value={newEvent.time} onChange={(e) => setNewEvent({ ...newEvent, time: e.target.value })} className="rounded-xl" />
              </div>
              <div className="grid gap-2">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsEventModalOpen(false)} className="rounded-xl">Ləğv Et</Button>
            <Button onClick={handleAddEvent} className="rounded-xl">Yadda Saxla</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <motion.div className="lg:col-span-2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>
          <Card className="glass-card border-none modern-shadow rounded-3xl h-full overflow-hidden">
            <CardHeader className="border-b border-primary/5 bg-primary/5 flex flex-row items-center justify-between">
              <div>
                <CardTitle className="text-2xl font-bold flex items-center gap-3"><CalendarIcon className="text-primary" size={28} />Region Operator Təqvimi</CardTitle>
                <CardDescription>Gündəlik sənəd və müraciət planı</CardDescription>
              </div>
              <Button onClick={() => setIsEventModalOpen(true)} className="rounded-xl bg-primary hover:bg-primary/90 text-white gap-2 shadow-lg shadow-primary/20">
                <Plus size={18} />
                Tədbir əlavə et
              </Button>
            </CardHeader>
                  ...
                  <div className="space-y-4">
                    {dayEvents.length > 0 ? dayEvents.map((event: any) => (
                      <div key={event.id} className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-primary/10 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
                        <div className={cn("absolute left-0 top-0 bottom-0 w-1", event.type === 'task' ? "bg-blue-500" : "bg-emerald-500")} />
                        <div className="flex justify-between items-start mb-1">
                          <p className="text-[10px] font-bold text-muted-foreground flex items-center gap-1"><Clock size={10} /> {event.time}</p>
                          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => handleEditEvent(event)} className="p-1 text-primary hover:bg-primary/5 rounded"><Edit3 size={12} /></button>
                            <button onClick={() => handleDeleteEvent(event.id)} className="p-1 text-rose-500 hover:bg-rose-50 rounded"><Trash2 size={12} /></button>
                          </div>
                        </div>
                    ...
                    )) : <div className="py-12 text-center opacity-30"><CalendarIcon size={32} className="mx-auto mb-2" /><p className="text-xs">Bu gün üçün plan yoxdur</p></div>}
                  </div>
                </div>
              </div>
            </CardContent>
        ...
        <motion.div className="space-y-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }}>
          ...
          <Card className="glass-card border-none modern-shadow rounded-3xl overflow-hidden flex flex-col max-h-[400px]">
            <CardHeader className="pb-3 bg-gradient-to-r from-primary/10 to-transparent">
              <div className="flex items-center justify-between mb-4">
                <CardTitle className="text-xl flex items-center gap-2"><StickyNote size={20} className="text-primary" />Şəxsi Qeydlər</CardTitle>
                <span className="text-[10px] font-bold bg-primary/10 text-primary px-2 py-1 rounded-full uppercase">{notes.length} qeyd</span>
              </div>
              <div className="relative w-full">
                <Input 
                  placeholder="Vacib qeyd yazın..." 
                  value={newNote} 
                  onChange={(e) => setNewNote(e.target.value)} 
                  onKeyDown={(e) => e.key === 'Enter' && handleAddNote()} 
                  className="rounded-2xl h-12 text-sm bg-white/50 border-primary/10 pl-4 pr-12 w-full shadow-inner" 
                />
                <Button 
                  size="sm" 
                  onClick={handleAddNote} 
                  className="absolute right-1 top-1 rounded-xl h-10 w-10 p-0 bg-primary hover:bg-primary/90 shadow-lg"
                >
                  <Plus size={16} />
                </Button>
              </div>
                  ...
                  <motion.div key={note.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={cn("p-4 rounded-2xl border transition-all group relative", inlineEditingId === note.id ? "bg-white border-primary/30" : "bg-white/60 dark:bg-slate-900/40 border-primary/5 hover:border-primary/20")}>
                    {inlineEditingId === note.id ? (
                      <div className="space-y-2">
                        <textarea autoFocus className="w-full text-xs bg-transparent border-none focus:ring-0 resize-none min-h-[40px]" value={inlineNoteText} onChange={(e) => setInlineNoteText(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); saveInlineEdit(); } if (e.key === 'Escape') setInlineEditingId(null); }} />
                        <div className="flex justify-end gap-2"><button onClick={() => setInlineEditingId(null)} className="text-[10px] text-muted-foreground">Ləğv et</button><button onClick={saveInlineEdit} className="text-[10px] font-bold text-primary">Saxla</button></div>
                      </div>
                    ) : (
                      <div className="flex items-start justify-between gap-4">
                        <p className="text-xs leading-relaxed flex-1">{note.text}</p>
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => { setInlineEditingId(note.id); setInlineNoteText(note.text); }} className="p-1 text-primary hover:bg-primary/5 rounded"><Edit3 size={10} /></button>
                          <button onClick={() => handleDeleteNote(note.id)} className="p-1 text-rose-500 hover:bg-rose-50 rounded"><Trash2 size={10} /></button>
                        </div>
                      </div>
                    )}
                  </motion.div>
          ...
          <Card className="glass-card border-none modern-shadow rounded-3xl h-full flex flex-col min-h-[300px]">
            ...
            <CardContent className="space-y-4 flex-1">
              {(dashboard?.overview?.recent_activity ?? []).slice(0, 5).map((activity: any, i: number) => (
                <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-muted/20 border border-transparent hover:border-primary/10">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-muted-foreground mt-1">{activity.timestamp_human}</p>
                  </div>
                </div>
              ))}
            </CardContent>
