...
export const ModernSchoolAdminDashboard = memo(() => {
  ...
  const [newEvent, setNewEvent] = React.useState({ title: '', type: 'event', time: '08:00', link: '' });
  
  const [myEvents, setMyEvents] = React.useState(() => {
    const saved = localStorage.getItem('tis_school_events');
    if (saved) {
      try {
    ...
    return [
      { id: 1, date: new Date(), title: "Pedaqoji şuranın iclası", type: "meeting", time: "08:30", link: "" },
      { id: 2, date: new Date(), title: "Açıq dərs (Riyaziyyat)", type: "event", time: "11:00", link: "" }
    ];
  });

  // Notes State
  const [notes, setNotes] = React.useState(() => {
    const saved = localStorage.getItem('tis_school_notes');
  ...
  React.useEffect(() => {
    localStorage.setItem('tis_school_events', JSON.stringify(myEvents));
  }, [myEvents]);

  React.useEffect(() => {
    localStorage.setItem('tis_school_notes', JSON.stringify(notes));
  }, [notes]);
  ...
  const handleAddEvent = () => {
    ...
    setNewEvent({ title: '', type: 'event', time: '08:00', link: '' });
  ...
  if (statsError) {
    return (
      <div className="p-12 text-center glass-card rounded-3xl m-6">
        ...
        <p className="text-muted-foreground mt-2">Məktəb məlumatlarını yükləmək mümkün olmadı.</p>
      </div>
    );
  }
  ...
  return (
    <ModernDashboardWrapper showGreeting={true}>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 mb-8">
        <ModernStatsCard title="Davamiyyət" value={stats?.today_attendance_rate ? `${stats.today_attendance_rate}%` : "0%"} icon={Users} trend={{ value: 0, isPositive: true }} color="blue" />
        <ModernStatsCard title="Aktiv İşlər" value={stats?.active_tasks || 0} icon={ClipboardList} trend={{ value: 0, isPositive: true }} color="emerald" />
        <ModernStatsCard title="Sorğular" value={stats?.pending_surveys || 0} icon={ListChecks} trend={{ value: 0, isPositive: true }} color="violet" />
        <ModernStatsCard title="Təcili" value={stats?.today_priority_items || 0} icon={AlertCircle} trend={{ value: 0, isPositive: true }} color="rose" />
      </div>
      ...
      <Dialog open={isEventModalOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="sm:max-w-[425px] rounded-3xl">
          <DialogHeader>
            ...
            <DialogDescription>{selectedDate ? format(selectedDate, "d MMMM yyyy", { locale: az }) : ""} üçün məktəb planı.</DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid gap-2"><Label htmlFor="title">Hadisə</Label><Input id="title" placeholder="Məs: İclas, Tədbir..." value={newEvent.title} onChange={(e) => setNewEvent({ ...newEvent, title: e.target.value })} className="rounded-xl" /></div>
            <div className="grid gap-2"><Label htmlFor="link">Keçid Linki</Label><Input id="link" placeholder="Məs: Zoom, Sənəd..." value={newEvent.link} onChange={(e) => setNewEvent({ ...newEvent, link: e.target.value })} className="rounded-xl" /></div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="grid gap-2"><Label htmlFor="type">Növ</Label>
                <select className="flex h-10 w-full rounded-xl border border-input bg-background px-3 py-2 text-sm" value={newEvent.type} onChange={(e) => setNewEvent({ ...newEvent, type: e.target.value })}>
                  ...
                  <option value="lesson">Açıq Dərs</option>
                </select>
              </div>
            </div>
          </div>
        ...
        <motion.div className="lg:col-span-2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>
          <Card className="glass-card border-none modern-shadow rounded-3xl h-full overflow-hidden">
            <CardHeader className="border-b border-primary/5 bg-primary/5 flex flex-row items-center justify-between">
              <div><CardTitle className="text-2xl font-bold flex items-center gap-3"><CalendarIcon className="text-primary" size={28} />Məktəb İdarəetmə Təqvimi</CardTitle><CardDescription>Dərslər, tədbirlər və iclaslar</CardDescription></div>
                  ...
                  <div className="space-y-4">
                    {dayEvents.length > 0 ? dayEvents.map((event: any) => (
                      <div key={event.id} className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-primary/10 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
                        <div className={cn("absolute left-0 top-0 bottom-0 w-1", event.type === 'meeting' ? "bg-rose-500" : "bg-emerald-500")} />
        ...
        <motion.div className="space-y-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }}>
          <Card className="glass-card border-none modern-shadow rounded-3xl overflow-hidden flex flex-col max-h-[400px]">
            <CardHeader className="pb-3 bg-gradient-to-r from-primary/10 to-transparent">
              <div className="flex items-center justify-between mb-4"><CardTitle className="text-xl flex items-center gap-2"><StickyNote size={20} className="text-primary" />Direktor Qeydləri</CardTitle><span className="text-[10px] font-bold bg-primary/10 text-primary px-2 py-1 rounded-full uppercase">{notes.length} qeyd</span></div>
          ...
          <Card className="glass-card border-none modern-shadow rounded-3xl h-full flex flex-col min-h-[300px]">
            <CardHeader><CardTitle className="text-xl flex items-center gap-2"><Zap size={20} className="text-amber-500" />Son Hərəkətlər</CardTitle></CardHeader>
            <CardContent className="space-y-4 flex-1">
              {(activities ?? []).slice(0, 5).map((activity: any, i: number) => (
                <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-muted/20 border border-transparent hover:border-primary/10">
                  ...
                  <div className="flex-1 min-w-0"><p className="text-xs font-medium leading-tight">{activity.title}</p><p className="text-[10px] text-muted-foreground mt-1">{activity.user_name || 'İstifadəçi'}</p></div>
                </div>
              ))}
            </CardContent>
          </Card>
