...
export function ReportTableStatisticsView() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'completed':
        return <Badge className="bg-emerald-100 text-emerald-800 border-emerald-200 whitespace-nowrap"><CheckCircle2 className="h-3 w-3 mr-1" /> Tamamlanıb</Badge>;
      case 'pending':
        return <Badge className="bg-amber-100 text-amber-800 border-amber-200 whitespace-nowrap"><Clock className="h-3 w-3 mr-1" /> Gözləyir</Badge>;
      case 'partial':
        return <Badge className="bg-blue-100 text-blue-800 border-blue-200 whitespace-nowrap"><BarChart3 className="h-3 w-3 mr-1" /> Qismən</Badge>;
      case 'draft':
        return <Badge className="bg-gray-100 text-gray-800 border-gray-200 whitespace-nowrap"><AlertCircle className="h-3 w-3 mr-1" /> Qaralama</Badge>;
      case 'not_started':
      default:
        return <Badge variant="outline" className="text-gray-400 border-dashed border-gray-300 whitespace-nowrap"><XCircle className="h-3 w-3 mr-1" /> Başlanmayıb</Badge>;
    }
  };
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 lg:h-[calc(100vh-200px)] min-h-0">
      {/* Left Panel - Tables List */}
      <div className="w-full lg:w-[300px] xl:w-[320px] shrink-0 flex flex-col gap-3 lg:h-full">
        ...
        <div className="bg-white border rounded-xl flex flex-col shadow-sm overflow-hidden h-[260px] lg:h-auto lg:flex-1">
            ...
            <div className="divide-y">
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg shrink-0 ${selectedTableId === 'overall' ? 'bg-indigo-100 text-indigo-600' : 'bg-gray-100 text-gray-400'}`}>
      ...
      {/* Right Panel */}
      <div className="flex-1 flex flex-col min-w-0 bg-white border rounded-xl shadow-sm lg:h-full overflow-hidden">
        {selectedTableId ? (
          <div className="flex flex-col h-full">
            ...
            <div className="p-4 bg-gray-50/50 border-b shrink-0">
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-4">
                <div className="flex items-start gap-3 min-w-0">
                  <div className={`p-2.5 rounded-xl shrink-0 ${isOverall ? 'bg-indigo-100 text-indigo-600' : 'bg-blue-100 text-blue-600'}`}>
                    {isOverall ? <Trophy className="h-5 w-5 sm:h-6 sm:w-6" /> : <BarChart3 className="h-5 w-5 sm:h-6 sm:w-6" />}
                  </div>
                  <div className="min-w-0">
                    <h2 className="font-extrabold text-base sm:text-xl text-gray-800 leading-tight">
                ...
                <Button
                  ...
                  size="sm"
                  className={`gap-2 shrink-0 w-full sm:w-auto ${isOverall ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-blue-600 hover:bg-blue-700'}`}
                >
              ...
              {kpis && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
                  <Card className="shadow-none border-gray-200">
                    <CardContent className="p-3">
                      <div className="flex items-center gap-1.5 text-gray-500 mb-1">
                        ...
                        <span className="text-[10px] uppercase tracking-wider font-semibold truncate">{kpis.labelTitle}</span>
                      </div>
                  ...
                  <Card className="shadow-none border-gray-200">
                    <CardContent className="p-3">
                      <div className="flex items-center gap-1.5 text-emerald-600 mb-1">
                        ...
                        <span className="text-[10px] uppercase tracking-wider font-semibold truncate">{kpis.labelFilled}</span>
                      </div>
                  ...
                  <Card className="shadow-none border-gray-200">
                    <CardContent className="p-3">
                      <div className="flex items-center gap-1.5 text-red-500 mb-1">
                        ...
                        <span className="text-[10px] uppercase tracking-wider font-semibold truncate">Doldurmayıb</span>
                      </div>
                  ...
                  <Card className={`shadow-none border-opacity-50 ${isOverall ? 'border-indigo-200 bg-indigo-50/30' : 'border-blue-200 bg-blue-50/30'}`}>
                    <CardContent className="p-3">
                      <div className={`flex items-center gap-1.5 mb-1 ${isOverall ? 'text-indigo-600' : 'text-blue-600'}`}>
                        <Target className="h-3.5 w-3.5 shrink-0" />
                        <span className="text-[10px] uppercase tracking-wider font-semibold truncate">Ümumi Faiz</span>
                      </div>
            ...
            {/* Filters */}
            <div className="p-3 border-b flex flex-col sm:flex-row sm:items-center justify-between gap-2 bg-white/80 backdrop-blur shrink-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <select
                  ...
                  className="h-9 px-3 rounded-md border border-input bg-background text-sm ring-offset-background focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 min-w-[120px]"
                >
              ...
              <div className="hidden sm:flex items-center gap-2 text-[10px] text-gray-500 bg-gray-50 px-2 py-1.5 rounded border border-dashed shrink-0">
              ...
              {isLoadingTableStats || isLoadingSchoolStats ? (
                ...
                <Table className="min-w-[680px]">
                  <TableHeader className="bg-white sticky top-0 z-20 shadow-[0_1px_0_rgba(0,0,0,0.05)]">
                    <TableRow>
                      <TableHead className="w-[44px] text-center bg-white text-gray-400 sticky left-0 z-30">#</TableHead>
                      <TableHead className="min-w-[200px] bg-white sticky left-[44px] z-30 border-r border-gray-200 shadow-[2px_0_6px_rgba(0,0,0,0.06)]">Məktəb / Sektor</TableHead>
                      <TableHead className="min-w-[110px] bg-white">{isOverall ? 'Doldurma' : 'Status'}</TableHead>
                      <TableHead className="min-w-[90px] bg-white">Hazırladı</TableHead>
                      <TableHead className="text-center w-[65px] bg-white">{isOverall ? 'Cəmi Sətir' : 'Sətir'}</TableHead>
                      <TableHead className="text-center w-[65px] bg-white">Təsdiq</TableHead>
                      <TableHead className="text-center w-[75px] text-red-600 bg-white font-bold">Rədd/Geri</TableHead>
                      <TableHead className="text-center w-[85px] bg-white">Bonus/Cərimə</TableHead>
                      <TableHead className="text-right min-w-[110px] bg-white">Reytinq (Bal)</TableHead>
                      {!isOverall && <TableHead className="w-[44px] bg-white"></TableHead>}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {filteredSchools.map((school, idx) => {
                      ...
                      const isStriped = !isOverall && !school.is_filled;
                      const stickyBg = isStriped ? 'bg-gray-50' : 'bg-white';

                      return (
                        <TableRow key={school.institution_id} className={isStriped ? 'bg-gray-50/30' : ''}>
                          <TableCell className={`text-center w-[44px] sticky left-0 z-10 ${stickyBg}`}>
                            {rank === 1 ? (
                          ...
                          <TableCell className={`sticky left-[44px] z-10 border-r border-gray-100 shadow-[2px_0_6px_rgba(0,0,0,0.04)] ${stickyBg}`}>
                          ...
                          <TableCell>
                            <span className="text-xs text-gray-600 font-medium whitespace-nowrap">{school.respondent_name || '-'}</span>
                          </TableCell>
                          ...
                          <TableCell className="text-center">
                            <div className="flex flex-col gap-0.5 items-center">
                              {(bonus ?? 0) > 0 && (
                                <span className="text-[10px] text-emerald-600 font-bold whitespace-nowrap">+{bonus?.toFixed(1)}</span>
                              )}
                              {(penalty ?? 0) > 0 && (
                                <span className="text-[10px] text-red-600 font-bold whitespace-nowrap">-{penalty?.toFixed(1)}</span>
                              )}
                          ...
                          <TableCell className="text-right">
                            <div className="flex flex-col items-end">
                              <div className="flex items-center gap-1.5 font-black text-base whitespace-nowrap">
                              ...
                              <div className="flex items-center gap-1 mt-0.5">
                                ...
                                <span className="text-[9px] font-bold text-gray-500 whitespace-nowrap">{percentage ?? 0}%</span>
                              </div>
                            </div>
                          </TableCell>
                          {!isOverall && (
              ...
              <ScrollArea className="h-[400px]">
                <div className="space-y-4 p-1">
                  ...
                  {responseDetail.rows.filter((_: any, idx: number) => {
                    const st = responseDetail.row_statuses?.[idx] as RowStatusMeta;
