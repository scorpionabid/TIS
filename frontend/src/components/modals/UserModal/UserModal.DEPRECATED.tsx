...
export function UserModal({
  ...
  const handleSubmit = async (data: any) => {
    ...
    try {
      ...
      // ✅ RegionOperator permission validation (minimum 1 required)
      if (finalData.role_name === 'regionoperator') {
        const hasAtLeastOnePermission = REGION_OPERATOR_PERMISSIONS.some(
          perm => finalData[perm.key] === true
        );

        if (!hasAtLeastOnePermission) {
          toast({
            title: 'Səlahiyyət Tələb Olunur',
            description: 'RegionOperator üçün ən azı 1 səlahiyyət seçilməlidir',
            variant: 'destructive',
          });
      ...
      >
        ...
        <Tabs value={state.activeTab} onValueChange={state.setActiveTab}>
          ...
          <div className="mt-6">
            ...
            {/* RegionOperator Permission Templates */}
            {state.activeTab === 'basic' && state.selectedRole && fields.isRegionalOperatorRole(state.selectedRole) && (
              <div className="mb-4 p-4 bg-indigo-50 rounded-lg border border-indigo-200">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <p className="text-sm font-medium text-indigo-900">🚀 Sürətli Səlahiyyət Seçimi</p>
                    <p className="text-xs text-indigo-600 mt-0.5">Hazır şablonlardan birini seçin və ya aşağıda fərdi təyin edin</p>
                  </div>
                </div>
                <div className="flex gap-2">
                  {Object.entries(PERMISSION_TEMPLATES).map(([key, template]) => (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        // Apply template permissions to form
                      ...
                      className="flex-1 px-3 py-2 text-sm border rounded-lg hover:bg-indigo-100 hover:border-indigo-400 transition-colors text-left"
                      title={template.description}
                    >
                      <div className="font-medium text-indigo-900">{template.label}</div>
                      <div className="text-xs text-indigo-600 mt-0.5">{template.description}</div>
                    </button>
                  ))}
                </div>
              </div>
...
// Export new role-based tabs modal for RegionAdmin user management
