...
const ASSIGNABLE_TO_FLAG_MAP: Record<string, string> = (() => {
  ...
  const modules = regionOperatorPermissions.modules as Record<string, RegionOperatorModuleConfig>;
...
export function transformFormDataToBackend(
  ...
  if (data.role_name === 'regionoperator') {
    ...
    if (Array.isArray(data.assignable_permissions) && data.assignable_permissions.length > 0) {
...
export function transformBackendDataToForm(user: any | null): Record<string, any> {
  ...
  const hasAssignable =
    Array.isArray(user.assignable_permissions) && user.assignable_permissions.length > 0;
  ...
  if (user.role_name === 'regionoperator') {
    ...
    console.log('🔍 [transformBackendDataToForm] RegionOperator permissions merged:', {
  ...
  } else if (!hasAssignable && user.region_operator_permissions) {
