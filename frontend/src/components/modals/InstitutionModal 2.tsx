...
const institutionTypes = [
  ...
  { value: 'regional', label: 'Regional İdarə' },
...
export const InstitutionModal: React.FC<InstitutionModalProps> = ({
  ...
  const { data: parentInstitutions } = useQuery({
    ...
    queryFn: () => {
      if (formData.type === 'ministry') return Promise.resolve({ data: [] });
      
      const parentType = 
        formData.type === 'regional' ? 'ministry' :
        formData.type === 'sector' ? 'regional' :
        formData.type === 'school' ? 'sector' : null;
      
      if (!parentType) return Promise.resolve({ data: [] });
      
      return institutionService.getByType(parentType as any);
    },
