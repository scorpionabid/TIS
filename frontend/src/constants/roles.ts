...
export const ROLE_DISPLAY_NAMES: Record<UserRole, string> = {
  ...
  [USER_ROLES.PRESCHOOLADMIN]: "Bağça İdarəçisi",
...
export const ROLE_GROUPS = {
  ...
  PRESCHOOL_ROLES: [USER_ROLES.PRESCHOOLADMIN],
...
export const isAdminRole = (role: UserRole): boolean => {
  return (ROLE_GROUPS.ADMIN_ROLES as readonly string[]).includes(role);
};

export const isManagementRole = (role: UserRole): boolean => {
  return (ROLE_GROUPS.MANAGEMENT_ROLES as readonly string[]).includes(role);
};

export const isSchoolRole = (role: UserRole): boolean => {
  return (ROLE_GROUPS.SCHOOL_ROLES as readonly string[]).includes(role);
};
