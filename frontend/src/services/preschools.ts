...
class PreschoolsService {
  ...
  ): Promise<{ success: boolean; data: Preschool[] }> {
    console.log(
      "🔍 PreschoolsService.getPreschools called with filters:",
      filters,
    );
    try {
  ...
  async createPreschool(
    data: PreschoolCreateData,
  ): Promise<{ success: boolean; message: string; data: Preschool }> {
  ...
  async updatePreschool(
    id: number,
    data: Partial<PreschoolUpdateData>,
  ): Promise<{ success: boolean; message: string; data: Preschool }> {
    console.log(
      "🔍 PreschoolsService.updatePreschool called for ID:",
      id,
      "with data:",
      data,
    );
    try {
      const response = await apiClient.put<Preschool>(
        `${this.baseUrl}/${id}`,
        data,
      );
  ...
  async getPreschoolStatistics(): Promise<{
    success: boolean;
    data: PreschoolStatistics;
  }> {
    try {
      const response = await apiClient.get<PreschoolStatistics>(
      ...
      console.error(
        "❌ PreschoolsService.getPreschoolStatistics failed:",
        error,
      );
      throw error;
    }
  ...
  async assignManager(
    ...
    console.log(
      "🔍 PreschoolsService.assignManager called for preschool:",
      preschoolId,
      "manager:",
      managerId,
    );
    try {
      const response = await apiClient.put<Preschool>(
        `${this.baseUrl}/${preschoolId}/assign-manager`,
  ...
  getMockStatistics(): PreschoolStatistics {
    return {
      ...
      by_sector: [
        ...
        {
          sector_id: 10,
          sector_name: "Gəncə Mərkəz Təhsil Sektoru",
          preschool_count: 4,
          active_count: 3,
        },
        {
          sector_id: 11,
          sector_name: "Gəncə Kəpəz Təhsil Sektoru",
          preschool_count: 2,
          active_count: 2,
        },
      ],
  ...
  // Import/Export methods for preschools
  async downloadImportTemplate(type: string = "kindergarten"): Promise<Blob> {
    return fetchBlob(`/institutions/template/${type}`, {
      errorMessage: "Template yüklənərkən xəta baş verdi",
    });
  }

  async importPreschools(
    file: File,
    type: string = "kindergarten",
  ): Promise<any> {
  ...
  async exportPreschools(type: string = "kindergarten"): Promise<Blob> {
    return fetchBlob(`/institutions/export?type=${type}`, {
      errorMessage: "İxrac zamanı xəta baş verdi",
    });
  }
}
