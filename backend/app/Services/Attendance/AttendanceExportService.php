...
class AttendanceExportService
{
    public function __construct(
        private readonly RegionalAttendanceService $attendanceService
    ) {}
    ...
    {
        $data = $this->attendanceService->getGradeLevelStats($user, $filters);
    ...
    {
        $data = $this->attendanceService->getSchoolGradeStats($user, $filters);

        $headers = ['Məktəb adı'];
        for ($i = 0; $i <= 11; $i++) {
            $headers[] = $this->toRomanNumeral($i);
        }
        $headers[] = 'Orta';
    ...
    {
        $data = $this->attendanceService->getSchoolsWithMissingReports($user, $filters);
