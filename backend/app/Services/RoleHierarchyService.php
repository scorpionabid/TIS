...
class RoleHierarchyService
{
    ...
    public function getPermissionScopeForLevel(int $level): array
    {
        ...
        $permissions = Cache::remember('permissions_by_scope', 3600, function () {
            return Permission::all()->groupBy(function ($permission) {
                ...
                if (str_contains($permission->name, 'system')) {
                    return 'system';
                }
                if (str_contains($permission->name, 'reports') || str_contains($permission->name, 'analytics')) {
                    return 'global';
                }
                if (str_contains($permission->name, 'institutions')) {
                    return 'regional';
                }
                if (str_contains($permission->name, 'users') && str_contains($permission->name, 'manage')) {
                    return 'sector';
                }
                if (str_contains($permission->name, 'surveys')) {
                    return 'institution';
                }
    ...
    public function getUserManageableRoles(User $user): Collection
    {
        ...
        return Role::where(function (Builder $query) use ($userLevel, $user) {
