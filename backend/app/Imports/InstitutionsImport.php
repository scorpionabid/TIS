...
class InstitutionsImport implements ToModel, WithBatchInserts, WithChunkReading, WithHeadingRow, WithValidation
{
    ...
    public function model(array $row)
    {
        try {
            ...
            if (! empty($row['utis_code'])) {
                ...
                // Validate UTIS code format (must be 8-12 digits)
                if (! preg_match('/^\d{8,12}$/', $utisCode)) {
                    Log::warning('Invalid UTIS code format (must be 8-12 digits):', ['utis_code' => $utisCode, 'row' => $row]);
                    $this->errors[] = "Row {$row['name']}: UTIS kod 8-12 rəqəmli olmalıdır ({$utisCode})";
    ...
    public function rules(): array
    {
        return [
            ...
            'utis_code' => 'nullable|string|regex:/^\d{8,12}$/|unique:institutions,utis_code',
        ];
    }
